...
Deno.serve(async(req)=>{try{
  ...
  if(org.activation_paid_at)return json({error:"activation_already_paid"},409);
  ...
  const externalReference=`srp_activation:${org.id}`;
  const response=await fetch("https://api.mercadopago.com/checkout/preferences",{method:"POST",headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json","X-Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({items:[{title:`Implantação Sr. Padeiro - ${org.name}`,quantity:1,currency_id:"BRL",unit_price:199}],payer:{email:user.email},external_reference:externalReference,back_urls:{success:"https://srpadeiro.gritnews.com.br/app?payment=success",pending:"https://srpadeiro.gritnews.com.br/app?payment=pending",failure:"https://srpadeiro.gritnews.com.br/app?payment=failure"},auto_return:"approved",notification_url:"https://pcrwtoddavpvkaxwtstc.supabase.co/functions/v1/mercadopago-webhook"})});
  const data=await response.json();if(!response.ok)return json({error:"mercado_pago_error",mode,provider_status:response.status},502);
  const checkoutUrl=mode==="test"?(data.sandbox_init_point||data.init_point):data.init_point;
  await sb.from("srp_billing_transactions").insert({organization_id:org.id,kind:"activation",provider_id:String(data.id),status:"pending",amount:199,currency:"BRL",external_reference:externalReference,checkout_url:checkoutUrl||null,payload:{...data,_integration_mode:mode}});
