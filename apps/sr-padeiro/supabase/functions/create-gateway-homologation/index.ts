...
const cors={"Access-Control-Allow-Origin":"https://srpadeiro.gritnews.com.br","Access-Control-Allow-Headers":"authorization, x-client-info, apikey, content-type","Access-Control-Allow-Methods":"POST, OPTIONS","Vary":"Origin"};
...
Deno.serve(async(req)=>{try{
  ...
  if(!response.ok)return json({error:"mercado_pago_error",provider_status:response.status,provider_code:String(data?.error||data?.message||"rejected")},502);
