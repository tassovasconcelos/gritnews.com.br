...
Deno.serve(async(req)=>{
  try{
    ...
    const {data:org}=await sb.from("srp_organizations").select("id,name").eq("id",organization_id).maybeSingle();
    ...
    const body={
      ...
      auto_recurring:{frequency:1,frequency_type:"months",transaction_amount:99,currency_id:"BRL"},
    ...
    await sb.from("srp_subscriptions").upsert({organization_id:org.id,plan_code:"sr_padeiro_99",provider:"mercadopago",provider_subscription_id:String(data.id),status:data.status==="authorized"?"active":"pending",provider_status:data.status,monthly_amount:99,payer_email:user.email,checkout_url:checkoutUrl||null,updated_at:new Date().toISOString()},{onConflict:"organization_id"});
