-- Amplia a ficha Cliente 360 com indicadores operacionais sem expor dados ao anon.
...
returns table(
  ...
  credit_balance numeric,low_stock_products bigint
)
...
begin
  ...
  with order_stats as (
    select o.tenant_id,
      count(*) filter(where o.opened_at>=now()-interval '90 days') orders_90d,
      coalesce(sum(o.total) filter(where o.opened_at>=now()-interval '90 days' and o.status='closed'),0)::numeric revenue_90d,
      max(o.opened_at) last_activity,
      count(*) filter(where o.status='open') open_orders
  ...
  select t.id,t.name,t.phone,t.subscription_status,coalesce(t.setup_status,'approved'),
    ...
    coalesce(ps.products_count,0),coalesce(cs.customers_count,0),
    coalesce(os.open_orders,0),coalesce(cr.credit_balance,0),coalesce(ps.low_stock_products,0)
  ...
  left join customer_stats cs on cs.tenant_id=t.id
  left join credit_stats cr on cr.tenant_id=t.id
