...
create table if not exists public.lead_material_catalog(id uuid primary key default gen_random_uuid(),product_key text not null,segment text,profile_key text,funnel_stage text not null default 'new',title text not null,material_type text not null default 'landing',material_url text not null,conversion_goal text not null default 'reply',active boolean not null default true,created_at timestamptz not null default now(),updated_at timestamptz not null default now());
...
create policy "admins_manage_lead_material_catalog" on public.lead_material_catalog for all to authenticated using (exists(select 1 from public.admin_users au where au.user_id=auth.uid() and au.active=true)) with check (exists(select 1 from public.admin_users au where au.user_id=auth.uid() and au.active=true));
...
returns table(segment text,profile_key text,pain_focus text,offer_focus text,confidence numeric) language plpgsql stable as $$
declare n text:=lower(coalesce(p_business_name,'')); begin
...
create or replace function public.grit_apply_lead_segmentation() returns trigger language plpgsql as $$ declare c record; begin if new.segment is null or new.segment_source='rule' then select * into c from public.grit_classify_lead_segment(coalesce(new.product_key,new.product),coalesce(new.business_name,new.name),new.source,new.landing_page) limit 1; new.segment:=c.segment;new.profile_key:=c.profile_key;new.pain_focus:=c.pain_focus;new.offer_focus:=c.offer_focus;new.segment_confidence:=c.confidence;new.segment_source:=coalesce(new.segment_source,'rule'); end if; return new; end $$;
...
create or replace function public.grit_resolve_lead_material(p_lead_id uuid) returns table(title text,material_url text,conversion_goal text) language sql stable as $$ select m.title,m.material_url,m.conversion_goal from public.leads l join public.lead_material_catalog m on m.product_key=coalesce(l.product_key,l.product) and m.active=true where l.id=p_lead_id and (m.segment=l.segment or m.segment is null) and (m.profile_key=l.profile_key or m.profile_key is null) and (m.funnel_stage=coalesce(l.status,'new') or m.funnel_stage='new') order by (m.segment=l.segment)::int desc,(m.profile_key=l.profile_key)::int desc,(m.funnel_stage=coalesce(l.status,'new'))::int desc,m.created_at desc limit 1; $$;
