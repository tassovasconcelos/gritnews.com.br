...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: tenant } = await sb.from("tenants")
      .select("id,owner_user_id,setup_status,activation_paid_at,courtesy_type,courtesy_ends_at")
    ...
    const paymentConfirmed = Boolean(tenant.activation_paid_at);
    if ((action === "start" || action === "approve") && !paymentConfirmed && !courtesyActive) {
      return json({ error: "payment_required", setup_status: tenant.setup_status }, 409);
    }

    if (action === "start") {
      const stamp = now.toISOString();
      const { error } = await sb.from("setup_requests").upsert({
        tenant_id: tenantId,
    ...
    if (action === "approve") {
      ...
      const { error: tenantError } = await sb.from("tenants").update({
        setup_status: "approved",
        setup_approved_at: stamp,
        setup_approved_by: user.id,
        trial_started_at: stamp,
        trial_ends_at: trialEnds,
      }).eq("id", tenantId);
      if (tenantError) return json({ error: "tenant_update_failed", detail: tenantError.message }, 500);
      const { error: requestError } = await sb.from("setup_requests").upsert({
        tenant_id: tenantId,
        owner_user_id: tenant.owner_user_id,
        status: "approved",
        assigned_admin: user.id,
