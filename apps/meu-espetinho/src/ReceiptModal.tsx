...
function receiptHtml(order:Order,settings:Settings,paymentMethod:string){
  const rows=order.items.map(i=>`<tr><td>${i.qty}x ${i.name}</td><td style="text-align:right">${money(i.qty*i.unitPrice)}</td></tr>`).join('');
...
async function makeReceiptImage(order:Order,settings:Settings,paymentMethod:string){
  ...
  for(const item of order.items){wrapText(ctx,`${item.qty}x ${item.name}`,padding,y,420,22);ctx.textAlign='right';ctx.fillText(money(item.qty*item.unitPrice),width-padding,y);ctx.textAlign='left';y+=rowH}
