...
import SeoOpsCenter from './SeoOpsCenter';
...
export default function AdminLoginEntry(){
  ...
  const isSeoOps=path.startsWith('/admin/apps/seo');
  ...
  if(isSeoOps)return <SeoOpsCenter/>;
  if(isSecurityOps)return <SecurityOpsCenter/>;
  if(isGritLeads)return <GritLeadsCrm/>;
  if(isGritApps)return <div style={{position:'relative'}}><div style={{position:'fixed',right:18,bottom:18,zIndex:9999,display:'flex',gap:8,flexWrap:'wrap',justifyContent:'flex-end'}}><a href="/admin/apps/seo" style={{...quickStyle,background:'#1d4ed8'}}>SEO & Indexação</a><a href="/admin/apps/security" style={{...quickStyle,background:'#334155'}}>Segurança & Operação</a><a href="/admin/apps/leads" style={{...quickStyle,background:'#0f766e'}}>Leads & Oportunidades</a></div><GritAppsAdmin/></div>;
  ...
  return <div style={{position:'relative'}}>
    <div style={{position:'fixed',right:18,bottom:18,zIndex:9999,display:'flex',gap:8,flexWrap:'wrap',justifyContent:'flex-end'}}>
      <a href="/admin/apps/seo" style={{...quickStyle,background:'#1d4ed8'}}>SEO & Indexação</a>
